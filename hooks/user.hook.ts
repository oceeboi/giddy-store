...
/**
 * `UserService.redeemRewards`.
 *
 * On success: account loyalty points and store credit are changed by the
 * backend, and referral views can also depend on the same points model, so
 * both related caches are invalidated.
 */
export function useRedeemRewardsMutation(options?: MutationOptionsOf<RewardsRedeemData, void>) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async () => unwrapResult(await userService.redeemRewards()),
    onSuccess: (result, variables, onMutateResult, context) => {
      queryClient.invalidateQueries({ queryKey: userKeys.account() });
      queryClient.invalidateQueries({ queryKey: userKeys.referral() });
