...
import { sendLoginSuccess } from '@/lib/email/service';
...
export async function POST(req: NextRequest) {
  ...
  const loginLocation = audit_meta.ipAddress ? `IP ${audit_meta.ipAddress}` : 'Unknown location';
  const loginDevice = req.headers.get('user-agent') ?? 'Unknown device';
  ...
  void sendLoginSuccess(
    ...
    console.error('[Email] login success alert failed:', email_error);
  });
