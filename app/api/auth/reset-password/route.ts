...
import { sendPasswordChanged } from '@/lib/email/service';
...
export async function POST(req: NextRequest) {
  ...
  if (!validation_result.success) {
    const validation_issues = validation_result.error.issues.map((issue) => ({
      path: issue.path.map((segment: any) => String(segment)) as (string | number)[],
  ...
  const found_user = await User.findOne({
    ...
    .select('_id email username status passwordResetTokenExp')
    .exec();
  ...
  void sendPasswordChanged(
    ...
    console.error('[Email] password changed email failed:', email_error);
  });
