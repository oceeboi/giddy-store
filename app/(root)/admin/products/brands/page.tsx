...
import { features, brandColumns } from '@/components/columns/admin/brand-column';
import { BrandRowCard } from '@/components/comps';
import { useAdminBrandsQuery } from '@/hooks/use-catalog.hook';

import { flexRender, useTable } from '@tanstack/react-table';
import { useTanStackTableDevtools } from '@tanstack/react-table-devtools';

export default function AdminBrandsPage() {
  const { data, isLoading, isError, error } = useAdminBrandsQuery();

  const table = useTable(
    {
      key: 'admin-brand-table',
      debugTable: false,
      features,
  ...
  if (isError) {
    return (
      <div className="rounded-none border border-red-200 bg-red-50 p-6 text-center text-red-700 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-400">
        <p className="font-semibold">Failed to load brands</p>
    ...
    <section className="w-full font-archivo">
      ...
      <div className="flex flex-col gap-3 md:hidden">
        {isLoading ? (
          ...
          brands.map((brand) => <BrandRowCard key={brand.id} brand={brand} />)
        )}
      </div>
