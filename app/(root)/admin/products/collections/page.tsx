...
import { features, collectionColumns } from '@/components/columns/admin/collection-column';
import { CollectionRowCard } from '@/components/comps';
import { useAdminCollectionsQuery } from '@/hooks/use-catalog.hook';

import { flexRender, useTable } from '@tanstack/react-table';
import { useTanStackTableDevtools } from '@tanstack/react-table-devtools';

export default function AdminCollectionsPage() {
  const { data, isLoading, isError, error } = useAdminCollectionsQuery();

  const table = useTable(
    {
      key: 'admin-collection-table',
      debugTable: false,
      features,
      columns: collectionColumns as any,
      ...
      <div className="flex flex-col gap-3 md:hidden">
        {isLoading ? (
          ...
          collections.map((collection) => (
            <CollectionRowCard key={collection.id} collection={collection} />
          ))
        )}
      </div>
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, idx) => (
                <tr key={idx} className="animate-pulse">
                  {collectionColumns.map((_, colIdx) => (
