...
export type RewardsRedeemData = {
  redeemed: boolean;
  pointsUsed: number;
  creditAddedKobo: number;
  remainingPoints: number;
  newStoreCreditKobo: number;
};
...
export class UserService {
  ...
  async redeemRewards(): Promise<ServiceResult<RewardsRedeemData>> {
    try {
      const res = await this.post<{ data: RewardsRedeemData }>('user/rewards/redeem');
      return { success: true, data: res.data };
    } catch (error) {
      return {
        success: false,
        message: UserService.fromHttpError(error, 'Failed to redeem rewards.', {
          409: 'Not enough points to redeem yet.',
        }),
      };
    }
  }
