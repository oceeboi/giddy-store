...
import { CategoryUpdate } from '@/components/comps';
...
export const categoryColumns = [
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="font-mono text-[11px] text-neutral-500 truncate">{category.slug}</span>
          </div>
        </div>
      );
    },
  ...
  columnHelper.accessor('parent', {
    ...
    cell: ({ getValue }) => {
      const parent_name = getValue();
      if (!parent_name) {
        return (
          <span className="font-archivo text-[10px] uppercase tracking-wider text-neutral-400 font-medium">
            Root
          </span>
        );
      }
      return (
        <span className="font-archivo text-xs text-neutral-700 dark:text-neutral-300">
          {parent_name.name}
        </span>
      );
    },
  ...
  columnHelper.display({
    ...
    cell: ({ row }) => {
      const category = row.original;
      return <CategoryRowActionCell category={category} />;
    },
  }),
];

function CategoryRowActionCell({ category }: { category: CategoryData }) {
